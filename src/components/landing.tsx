        ...
        <Dialog>
          ...
          <DialogContent className="w-[94vw] max-w-[500px] bg-[#0f0f0f] border-primary/30 text-white rounded-[2rem] p-6 gap-4">
            ...
            <DialogFooter className="pt-4 flex flex-col-reverse sm:flex-row gap-3">
              <DialogClose asChild>
                <Button type="button" variant="ghost" className="w-full text-zinc-500 font-bold hover:text-white uppercase tracking-widest text-xs h-12" disabled={isNavigating}>
                  {t.cancel}
                </Button>
              </DialogClose>
              <Button 
                ...
                className={cn(
                  ...
                  (!agreed || isNavigating) && "opacity-50"
                )}
              >
                {isNavigating ? (
